...
def get_antenna_coordinates(map: list[str]) -> list[list[int|str]]:
...
def find_unique_antinode_locations(coordinates: list[list[int | str]], map: list[str]):
    ...
    for i, antenna in enumerate(coordinates):
        ...
        for match in coordinates[i+1:]:
            ...
            if match[0] == antenna[0]:
                ...
                if 0 <= antinode_x1 < len(map[0]) and 0 <= antinode_y1 < len(map) and (antinode_x1, antinode_y1) not in antinodes:
                    antinodes.append((antinode_x1, antinode_y1))
                antinode_x2, antinode_y2 = match[1] + x_diff, match[2] + y_diff
                if 0 <= antinode_x2 < len(map[0]) and 0 <= antinode_y2 < len(map) and (antinode_x2, antinode_y2) not in antinodes:
...
if __name__ == "__main__":
    ...
    coordinates = get_antenna_coordinates(data)
    answer_1 = find_unique_antinode_locations(coordinates, data)
    print(answer_1)
